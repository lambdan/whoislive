...
def streams(offset):
    url = "https://api.twitch.tv/kraken/streams/followed?client_id=" + clientid + \
        "&oauth_token=" + token + "&stream_type=live&offset=" + str(offset) + "&limit=25"
    try:
    ...
    for stream in data['streams']:
        ...
        print(Fore.GREEN + name, Fore.YELLOW + status, Fore.RED + viewers + " viewers", end="\n")
...
if not os.path.isfile(token_file):
    ...
    print("Testing token...")
    testURL = "https://api.twitch.tv/kraken?client_id=" + \
        clientid + "&oauth_token=" + input_token.rstrip()
...
if os.path.isfile(token_file):
    ...
    # get first 25 :) (yeah this is really weird structure wise but whatever)
    totalstreams = streams(0)
    offset = 25
